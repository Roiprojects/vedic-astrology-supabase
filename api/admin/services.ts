...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "GET") {
    if (slug) {
      supa.from("services").select("*").eq("slug", slug).maybeSingle().then(({ data, error }) => {
        if (error || !data) return res.status(404).json({ error: "Not found" });
        res.status(200).json({ service: data });
      });
      return;
    }
    supa.from("services").select("*").order("display_order", { ascending: true }).order("title", { ascending: true }).then(({ data, error }) => {
  ...
  if (req.method === "DELETE" && slug) {
    supa.from("services").delete({ count: "exact" }).eq("slug", slug).then(({ error, count }) => {
      if (error) return res.status(500).json({ error: "DB error" });
