...
export default function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === "GET") {
    if (slug) {
      supa.from("homams").select("*").eq("slug", slug).maybeSingle().then(({ data, error }) => {
        if (error || !data) return res.status(404).json({ error: "Not found" });
        res.status(200).json({ homam: data });
      });
      return;
    }
    supa.from("homams").select("*").order("display_order", { ascending: true }).order("name", { ascending: true }).then(({ data, error }) => {
      if (error) return res.status(500).json({ error: "DB error" });
      res.status(200).json({ homams: data || [] });
    });
    return;
  }
  ...
  if (req.method === "POST") {
    supa.from("homams").insert({
      ...
      res.status(200).json({ ok: true, slug: body.slug });
    });
    return;
  }

  if (req.method === "PUT" && slug) {
    supa.from("homams").update({
      slug: body.slug, name: body.name, icon: body.icon, image: body.image ?? null,
      short_benefit: body.shortBenefit, full_description: body.fullDescription,
      price: body.price, discount_price: body.discountPrice ?? null, duration: body.duration,
      gradient: body.gradient, benefits: body.benefits, suitable_for: body.suitableFor,
  ...
  if (req.method === "DELETE" && slug) {
    supa.from("homams").delete({ count: "exact" }).eq("slug", slug).then(({ error, count }) => {
      if (error) return res.status(500).json({ error: "DB error" });
